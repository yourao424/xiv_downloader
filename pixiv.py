...
pixiv_following_users_list_url = 'https://www.pixiv.net/bookmark.php?type=user&rest=show&p='
driver_path = './driver/chromedriver.exe'
json_path = './json/client.json'
info_path = './info/'
...
def set_filename(user_name, illust_title, illust_id):
    ...
    str_comp = re.compile(r'[/:;,\*"<>|　]')

    user_name = user_name.encode('utf8').decode('utf8')
    illust_title = illust_title.encode('utf8').decode('utf8')

    image_name = "{0} - {1} ({2}).png".format(str_comp.sub('',user_name), str_comp.sub('',illust_title), illust_id)
    return image_name
...
def get_following_users_info(pages, api, aapi):
    ...
    for page in range(1, pages+1):
        ...
        following_users_ids = browser.find_elements_by_xpath('//*[@id="search-result"]/div[1]/ul/li/div/a')
        for i, target_users_id in enumerate(following_users_ids):

            following_users_info = aapi.user_detail(int(target_users_id.get_attribute('data-user_id')))
            try:
                following_users_id.append(following_users_info.user.id)
                following_users_name.append(following_users_info.user.name)
                following_users_works.append(following_users_info.profile.total_illusts + following_users_info.profile.total_manga)
                following_users_url.append('https://www.pixiv.net/users/' + str(following_users_info.user.id))
            except:
                continue
...
def get_following_illust(api, aapi, input_path, output_path):
    ...
    for i in range(0, len(df_following_users_info) - 1):
        ...
        for work in range(0, target_user_total_illust):
            ...
            image_name = set_filename(target_user_name, target_illust.title, target_illust.id)
            ...
            # else:
            aapi.download(target_illust.image_urls.large, path=download_path, name=image_name)


def login_pixiv(client):

    browser.find_element_by_xpath('//*[@id="LoginComponent"]/form/div[1]/div[1]/input').send_keys(client['pixiv_id'])
    browser.find_element_by_xpath('//*[@id="LoginComponent"]/form/div[1]/div[2]/input').send_keys(client['password'])
    sleep(5)
